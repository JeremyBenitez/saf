...
class ventas_x_tiendas:
    ...
    """def diario_cashea(self): #Venta diaria por tiende metodo cashea
        ...
        cursor.execute('TotalVentasenBSxTiendaCSH @FechaInicio = ? , @FechaFin = ?,@c_Localidad = ?',self.fecha_ini, self.fecha_out,self.tienda)
...
v_bs = ventas_x_tiendas(str(fecha_diaria),str(fecha_diaria),'0206')
ventas_bs_valera = v_bs.diaro_bs()    

"""v_csh=ventas_x_tiendas(str(fecha_diaria),str(fecha_diaria),'0206')
ventas_csh_valera = v_csh.diario_cashea()"""

v_usd=ventas_x_tiendas(str(fecha_diaria),str(fecha_diaria),'0206')
ventas_usd_valera ,transacciones= v_usd.diario_usd()

v_usd_efe=ventas_x_tiendas(str(fecha_diaria),str(fecha_diaria),'0206')
ventas_efe_valera = v_usd_efe.diario_efectivo_usd()

print(ventas_usd_valera)